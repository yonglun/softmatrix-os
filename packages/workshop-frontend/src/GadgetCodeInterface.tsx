...
export default function GadgetCodeInterface({ overseer, filesRoot, height = '100%', onCodeChange, selectedChatId = null, proposedChanges, draftProposedChanges, streamingProposedChanges, streamingActiveFile, isAgentActive, isVisible = true, onHasCodeChange }: GadgetCodeInterfaceProps) {
    ...
    const subscribe = async () => {
      try {
        ...
        if (!isReadyRef.current) {
          toasts.add({ title: t('codeSurface.loadFailed'), variant: 'error' })
          setLoading(false)
        }
  ...
  const handleFileCreate = (filename: string) => {
    ...
    if (filesMap.has(filename)) {
      toasts.add({ title: t('codeSurface.fileAlreadyExists', { filename }), variant: 'error' })
      return
    }
    ...
    setActiveFile(filename)
    toasts.add({ title: t('codeSurface.fileCreated', { filename }), variant: 'success' })
  }
  ...
  const handleFileDelete = (filename: string) => {
    ...
    if (!filesMap.has(filename)) {
      toasts.add({ title: t('codeSurface.fileNotFound'), variant: 'error' })
      return
    }
    ...
    toasts.add({ title: t('codeSurface.fileDeleted', { filename }), variant: 'success' })
  }
  ...
  const handleFileRename = (oldName: string, newName: string) => {
    ...
    if (filesMap.has(newName)) {
      toasts.add({ title: t('codeSurface.fileAlreadyExists', { filename: newName }), variant: 'error' })
      return
    }
    ...
    toasts.add({ title: t('codeSurface.fileRenamed', { oldName, newName }), variant: 'success' })
  }
  ...
  const handleFileDownload = useCallback((filename: string) => {
    ...
    if (!ytext) {
      toasts.add({ title: t('codeSurface.couldNotDownload', { filename }), variant: 'error' })
      return
    }
  ...
  const activeFileModeLabel = isEditingLocked
    ? t('codeSurface.reviewingChangesIn')
    : isDiffMode
      ? t('codeSurface.editingChangesIn')
      : t('codeSurface.editing')
        ...
        <div className="flex flex-col bg-kumo-base" style={{ flex: 1, minWidth: 0 }}>
          {activeFile && (
            <div className="flex h-9 shrink-0 items-center justify-between gap-3 border-b border-kumo-line bg-kumo-base px-3">
              ...
              <WorkshopIconButton
                aria-label={t('codeSurface.download', { filename: activeFile })}
                title={t('codeSurface.downloadFile')}
