...
function formatRelativeTime(date: Date, t: (key: string, options?: Record<string, unknown>) => string): string {
  ...
  if (minutes < 60) return t('workspacesSurface.minutesAgo', { count: minutes })
  const hours = Math.floor(minutes / 60)
  if (hours < 24) return t('workspacesSurface.hoursAgo', { count: hours })
  const days = Math.floor(hours / 24)
  return t('workspacesSurface.daysAgo', { count: days })
}
      ...
      <DropdownMenu>
        <DropdownMenu.Trigger
          render={
            <button
              type="button"
              aria-label={t('outputsSurface.actions')}
        ...
        <DropdownMenu.Content className={MENU_CONTENT}>
          <DropdownMenu.Item onClick={onOpen} className={MENU_ITEM}>
            <ArrowSquareOut size={13} className="mr-2" /> {t('outputsSurface.open')}
          </DropdownMenu.Item>
          <DropdownMenu.Item onClick={onOpenWorkspace} className={MENU_ITEM}>
            <Cube size={13} className="mr-2" /> {t('outputsSurface.openWorkspace')}
          </DropdownMenu.Item>
          {onRename && (
            <DropdownMenu.Item onClick={onRename} className={MENU_ITEM}>
              <PencilSimple size={13} className="mr-2" /> {t('outputsSurface.rename')}
            </DropdownMenu.Item>
          )}
          {onRemove && (
            <DropdownMenu.Item onClick={onRemove} className={`${MENU_ITEM} text-kumo-danger`}>
              <Trash size={13} className="mr-2" /> {t('outputsSurface.remove')}
            </DropdownMenu.Item>
          )}
...
function subtitle(output: OutputSummary, t: (key: string, options?: Record<string, unknown>) => string): string {
  const parts = [output.workspaceTitle || t('outputsSurface.untitledWorkspace')]
  if (output.owner) parts.push(t('outputsSurface.sharedBy', { name: output.owner.name }))
  parts.push(t('outputsSurface.active', { relative: formatRelativeTime(output.lastActive, t) }))
  return parts.join(' · ')
}
...
function OutputProvenance({ owner }: { owner?: OutputSummary['owner'] }) {
  ...
  return (
    <span
      ...
      title={owner ? t('outputsSurface.inSharedWorkspace', { name: owner.name }) : t('outputsSurface.inCreatedWorkspace')}
    >
      {owner ? <ShareNetwork size={11} /> : <User size={11} />}
      <span className="truncate">{owner ? t('outputsSurface.sharedBy', { name: owner.name }) : t('outputsSurface.createdByYou')}</span>
    </span>
  )
}
      ...
      <div className="flex items-center gap-2.5 px-3 py-2.5">
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-[13px] font-medium leading-[18px] tracking-[-0.25px] text-kumo-default">
            {output.title || t('outputsSurface.untitled')}
          </p>
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 truncate text-[12px] leading-4 tracking-[-0.2px] text-kumo-subtle">
          {formatOf(output.output).noun} · {output.workspaceTitle || t('outputsSurface.untitledWorkspace')}
        </p>
      </div>
      ...
      <div className="hidden shrink-0 items-center gap-6 text-xs text-kumo-inactive lg:flex">
        ...
        <span className="flex w-40 items-center justify-end gap-1 whitespace-nowrap">
          <Clock size={10} />
          {t('outputsSurface.active', { relative: formatRelativeTime(output.lastActive, t) })}
        </span>
      </div>
      <OutputMenu onOpen={onOpen} onOpenWorkspace={onOpenWorkspace}
                  onRename={onRename} onRemove={onRemove} />
...
function ScopeSelect({
  ...
  const options: { value: OwnerFilter; label: string }[] = [
    { value: 'all', label: t('outputsSurface.yoursAndShared') },
    { value: 'mine', label: t('outputsSurface.createdByYou') },
    { value: 'shared', label: t('outputsSurface.sharedWithYou') },
  ]
      ...
      >
        <form onSubmit={(event) => { event.preventDefault(); onSave() }}>
          <div className="flex items-start justify-between gap-4 border-b border-kumo-line px-5 py-4">
            <div className="min-w-0">
              ...
              <Dialog.Description className="mt-1 text-[12px] leading-4 text-kumo-subtle">
                {t('outputsSurface.renameDescription', { workspace: output?.workspaceTitle || t('outputsSurface.untitledWorkspace') })}
              </Dialog.Description>
            </div>
          ...
          <div className="flex items-center justify-end gap-2 border-t border-kumo-line px-5 py-3">
            <WorkshopButton type="button" disabled={busy} onClick={onClose}>{t('outputsSurface.cancel')}</WorkshopButton>
            <WorkshopButton tone="primary" type="submit" disabled={busy || !value.trim()}>
              {busy ? t('outputsSurface.saving') : t('outputsSurface.save')}
            </WorkshopButton>
          </div>
        </form>
      </Dialog>
...
function OutputsPage() {
  const { t } = useTranslation()
  useDocumentTitle(t('outputsSurface.title'))
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (loadedOnce.current) {
        toastsRef.current.add({ title: t('outputsSurface.refreshFailed'), variant: 'error' })
      } else {
        setLoadError(true)
      }
    })
  ...
  const saveRename = async () => {
    ...
    try {
      ...
      toasts.add({ title: t('outputsSurface.renameFailed'), variant: 'error' })
    } finally {
  ...
  const confirmRemove = async () => {
    ...
    try {
      ...
      toasts.add({ title: t('outputsSurface.removeFailed'), variant: 'error' })
    } finally {
      gadget?.[Symbol.dispose]()
      overseer?.[Symbol.dispose]()
      setMutationBusy(false)
  ...
  return (
    <div className="mx-auto flex h-full w-full max-w-5xl flex-col px-6 sm:px-10">
      <header className="flex items-end justify-between gap-4 px-3 pb-4 pt-10">
        <div className="min-w-0">
          <h1 className="text-2xl font-semibold tracking-tight text-kumo-default">{t('outputsSurface.title')}</h1>
          <p className="mt-1 text-[13px] leading-[18px] tracking-[-0.25px] text-kumo-subtle">
            {t('outputsSurface.description')}
          </p>
        </div>
      ...
      <div className={`flex flex-col gap-3 px-3 pb-3 sm:flex-row sm:items-center sm:justify-between ${
        ...
        <div className="-mx-1 flex items-center gap-1 overflow-x-auto px-1 sidebar-scroll">
          {showTypeFilters && (
            <>
              <FilterChip active={typeFilter === 'all'} label={t('outputsSurface.all')} count={inTypeScope.length}
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <div className="relative sm:w-56">
            ...
            <input
              ...
              placeholder={t('outputsSurface.searchPlaceholder')}
      ...
      <div className="chat-panel min-h-0 flex-1 overflow-y-auto pb-8 pt-1">
        {loading ? (
          ...
          <div className="py-12 text-center text-sm">
            <p className="text-kumo-danger">{t('outputsSurface.loadError')}</p>
            <button onClick={() => setReloadToken((n) => n + 1)} className="mt-1 text-kumo-brand underline">
              {t('outputsSurface.retry')}
            </button>
          </div>
        ) : filtered.length === 0 ? (
          <div className="flex flex-col items-center gap-3 px-3 py-20 text-center">
            ...
            <div>
              <p className="text-sm font-medium text-kumo-default">
                {isFiltered ? t('outputsSurface.noMatch') : t('outputsSurface.noOutputs')}
              </p>
              <p className="mt-1 text-[13px] leading-[18px] text-kumo-subtle">
                {isFiltered
                  ? t('outputsSurface.filterHint')
                  : t('outputsSurface.emptyHint')}
              </p>
            </div>
            {/* Offer the deployment's formats here rather than sending them to the home page. */}
            {!isFiltered && <NewFormatRow label={t('outputsSurface.startWith')} />}
          </div>
        ) : view === 'grid' ? (
          <div className="grid grid-cols-2 gap-4 px-3 sm:grid-cols-3 lg:grid-cols-4">
            {filtered.map((output) => (
      ...
      <DeleteConfirmationDialog
        ...
        title={`${t('outputsSurface.removeConfirmTitle')} “${removeOutput?.title || t('outputsSurface.untitled')}”`}
        description={
          <>
            {t('outputsSurface.removeDescription')}
          </>
        }
        confirmLabel={t('outputsSurface.remove')}
        confirmingLabel={t('workspacesSurface.removing')}
