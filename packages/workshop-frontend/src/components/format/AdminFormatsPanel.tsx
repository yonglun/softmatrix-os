...
export default function AdminFormatsPanel({
  ...
  const mutate = async (op: () => Promise<void>) => {
    ...
    try {
      ...
      toasts.add({ title: t('management.formats.updateFailed'), variant: 'error' })
    } finally {
      setBusy(false)
    }
  }
  ...
  return (
    <div className="rounded-xl border border-kumo-line bg-kumo-elevated p-6">
      <h2 className="mb-1 text-lg font-semibold text-kumo-strong">{t('management.formats.title')}</h2>
      <p className="mb-5 text-sm text-kumo-subtle">
        {t('management.formats.description')}
      </p>
            ...
            >
              ...
              <span className="min-w-0">
                <span className="block truncate text-[13px] text-kumo-default">
                  {candidate.title || t('management.formats.untitled')}
                </span>
                <span className="block truncate text-[11px] text-kumo-inactive">
                  {candidate.declared
                    ? t('management.formats.produces', { plural: candidate.declared.plural })
                    : t('management.formats.noDeclared')}
                </span>
              </span>
            </DropdownMenu.Item>
          ))}
  ...
  return (
    <div className="mb-5 rounded-lg border border-dashed border-kumo-line bg-kumo-tint/40 p-4">
      <p className="mb-2.5 text-[11px] font-medium uppercase tracking-[0.06em] text-kumo-inactive">
        {t('management.formats.previewTitle')}
      </p>
      {formats.length === 0 ? (
        <p className="text-[13px] italic text-kumo-inactive">
          {t('management.formats.previewEmpty')}
        </p>
      ) : (
        <div className="flex flex-wrap items-center gap-2">
          {formats.map((format) => (
            ...
            >
              ...
              {t('management.formats.previewNew', { noun: format.output!.noun })}
            </span>
          ))}
        </div>
      )}
      <p className="mt-2.5 text-[12px] leading-4 text-kumo-subtle">
        {t('management.formats.previewHint')}
      </p>
    </div>
  )
}
...
function EmptyState() {
  ...
  return (
    <div className="mb-5 rounded-lg border border-kumo-line bg-kumo-base px-4 py-5 text-center">
      <p className="text-sm font-medium text-kumo-default">{t('management.formats.emptyTitle')}</p>
      <p className="mx-auto mt-1 max-w-md text-[13px] leading-[18px] text-kumo-subtle">
        {t('management.formats.emptyDescription')}
      </p>
    </div>
  )
}
  ...
  return (
    <div className="group rounded-lg border border-kumo-line bg-kumo-base">
      <div className="flex items-center gap-3 p-3">
        ...
        >
          {format.missing ? (
            <span
              ...
              title={t('management.formats.missingTitle')}
            >
              <Warning size={16} />
            </span>
          ) : (
          ...
          <span className="min-w-0 flex-1">
            <span className="flex items-center gap-2">
              <span className="truncate text-sm font-medium text-kumo-default">
                {format.output ? t('management.formats.previewNew', { noun: format.output.noun }) : format.blueprintTitle || format.blueprintId}
              </span>
              {format.bundled && <Badge>{t('management.formats.bundled')}</Badge>}
              {!format.enabled && !format.missing && <Badge>{t('management.formats.off')}</Badge>}
              {needsNaming && <Badge tone="warn">{t('management.formats.needsName')}</Badge>}
            </span>
            <span className="mt-0.5 block truncate text-xs text-kumo-subtle">
              {format.missing
                ? t('management.formats.blueprintDeleted')
                : needsNaming
                ? t('management.formats.noOutputName')
                : t('management.formats.shownUnder', { title: format.blueprintTitle, plural: format.output!.plural })}
            </span>
          </span>
        ...
        >
          <IconButton label={t('management.formats.moveUp')} disabled={busy || isFirst} onClick={() => onMove(-1)}>
            <ArrowUp size={13} />
          </IconButton>
          <IconButton label={t('management.formats.moveDown')} disabled={busy || isLast} onClick={() => onMove(1)}>
            <ArrowDown size={13} />
          </IconButton>
        </div>
      ...
      {open && (
        <div className="flex flex-col gap-4 border-t border-kumo-line px-3 py-4">
          {format.missing ? (
            ...
            <>
              <Fieldset
                title={t('management.formats.howPresented')}
                detail={
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="grid flex-1 gap-2 sm:grid-cols-[auto_1fr_1fr]">
                    ...
                    />
                    <OverrideField
                      label={t('management.formats.name')}
                    ...
                    />
                    <OverrideField
                      label={t('management.formats.plural')}
              ...
              <Fieldset
                title={t('management.formats.howAgentPicks')}
                detail={t('management.formats.standardCatalog')}
              >
                <OverrideField
                  label={t('management.formats.hint')}
                  placeholder={t('management.formats.hintPlaceholder')}
                  ...
                  <p className="mt-2 flex items-start gap-1.5 rounded-md bg-kumo-tint/60 px-2.5 py-2 font-mono text-[11px] leading-4 text-kumo-subtle">
                    ...
                    <span className="min-w-0">
                      <span className="block">
                        {t('management.formats.catalogEntry', { noun: format.output.noun })}
              ...
              <div className="flex items-end justify-between gap-4 border-t border-kumo-line pt-3">
                <p className="text-[12px] leading-4 text-kumo-subtle">
                  {(format.enabled
                    ? t('management.formats.turningOff')
                    : t('management.formats.hidden')) +
                    (format.bundled
                      ? t('management.formats.shipsWithDeployment')
                      : '')}
                </p>
                {!format.bundled && (
                  <Button variant="secondary" disabled={busy} onClick={onRemove}>
                    <Trash size={13} className="mr-1.5" />
                    {t('management.formats.stopOffering')}
                  </Button>
                )}
              </div>
            </>
  ...
  return (
    <label className="flex flex-col gap-1">
      <span className="text-[11px] font-medium uppercase tracking-[0.06em] text-kumo-inactive">
        {label}
        {overridden && (
          <span className="ml-1 normal-case tracking-normal text-kumo-subtle">{t('management.formats.overridden')}</span>
        )}
      </span>
      <Input
        value={draft}
        ...
        <DropdownMenu.Trigger
          render={
            <button
              type="button"
              disabled={disabled}
              aria-label={t('management.formats.chooseIcon')}
