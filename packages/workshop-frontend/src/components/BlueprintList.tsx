...
function formatRelativeTime(date: Date, t: (key: string, options?: Record<string, unknown>) => string): string {
  ...
  if (minutes < 1) return t('management.blueprintList.justNow')
  if (minutes < 60) return t('management.blueprintList.minutesAgo', { count: minutes })
  const hours = Math.floor(minutes / 60)
  if (hours < 24) return t('management.blueprintList.hoursAgo', { count: hours })
  const days = Math.floor(hours / 24)
  return t('management.blueprintList.daysAgo', { count: days })
}
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          <h3 className="truncate text-sm font-medium text-kumo-default">
            {item.title || t('management.blueprintList.untitled')}
          </h3>
        </div>
        {item.description && (
        ...
        <DropdownMenu>
          ...
          <DropdownMenu.Content className={MENU_CONTENT}>
            <DropdownMenu.Item onClick={() => onTogglePin(item)} className={MENU_ITEM}>
              ...
              {item.pinned ? t('management.blueprintList.unfavorite') : t('management.blueprintList.favorite')}
            </DropdownMenu.Item>
            {item.inLibrary && (
              <DropdownMenu.Item variant="danger" onClick={() => onRemoveFromLibrary(item)} className={MENU_ITEM_DANGER}>
                <Trash size={13} className="mr-2" />
                {t('management.blueprintList.removeFromLibrary')}
              </DropdownMenu.Item>
            )}
...
export default function BlueprintList() {
    ...
    Promise.all([authenticatedApi.listOwnBlueprints(), authenticatedApi.listLibraryBlueprints()])
      .then(([own, library]) => {
        ...
        const ensure = (id: string): BlueprintItem => {
          let it = map.get(id)
          if (!it) {
            it = { id, title: t('management.blueprintList.untitled'), description: '', recency: 0, pinned: false, inLibrary: false, isOwn: false }
            map.set(id, it)
          }
          return it
        }
  ...
  const handleBlueprintSelected = useCallback(async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toasts.add({ title: t('management.blueprintList.uploaded'), variant: 'success' })
      load()
    } catch (err) {
      console.error('Failed to upload blueprint:', err)
      toasts.add({ title: t('management.blueprintList.uploadFailed'), variant: 'error' })
    } finally {
      setUploading(false)
    }
  ...
  const handleTogglePin = async (item: BlueprintItem) => {
    ...
    try {
      ...
      toasts.add({ title: t('management.blueprintList.favoriteFailed'), variant: 'error' })
    }
  }
  ...
  const handleRemoveFromLibrary = async (item: BlueprintItem) => {
    try {
      ...
      toasts.add({ title: t('management.blueprintList.removedFromLibrary'), variant: 'success' })
    } catch (err) {
      console.error('Failed to remove blueprint from library:', err)
      toasts.add({ title: t('management.blueprintList.removeFailed'), variant: 'error' })
    }
  }
        ...
        <div className="mb-4 flex items-center gap-2 px-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder={t('management.blueprintList.search')}
          ...
          <div className="grid shrink-0 grid-cols-2 gap-2">
            <Link to="/explore" className={ACTION_BUTTON}>
              <Compass size={14} />
              {t('management.blueprintList.explore')}
            </Link>
            <button
              type="button"
              onClick={() => uploadInputRef.current?.click()}
              disabled={uploading}
              title={t('management.blueprintList.uploadArchive')}
              className={ACTION_BUTTON}
            >
              <UploadSimple size={14} weight="bold" />
              {uploading ? t('management.blueprintList.uploading') : t('management.blueprintList.upload')}
            </button>
          </div>
        </div>
      )}
      ...
      <div className="chat-panel flex min-h-0 flex-1 flex-col gap-0.5 overflow-y-auto pt-1">
        {loading ? (
          ...
          <div className="py-12 text-center text-sm">
            <p className="text-kumo-danger">{t('management.blueprintList.loadFailed')}</p>
            <button type="button" onClick={load} className="mt-1 text-kumo-brand underline">{t('management.blueprintList.tryAgain')}</button>
          </div>
        ) : filtered.length === 0 ? (
          search ? (
            <div className="py-12 text-center text-sm text-kumo-inactive">{t('management.blueprintList.noFound')}</div>
          ) : (
            <div className="flex flex-col items-center gap-3 px-3 py-16 text-center">
              ...
              <div>
                <p className="text-sm font-medium text-kumo-default">{t('management.blueprintList.noBlueprints')}</p>
                <p className="mt-1 text-[13px] leading-[18px] text-kumo-subtle">
                  {t('management.blueprintList.emptyDescription')}
                </p>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <Link to="/explore" className={ACTION_BUTTON}>
                  <Compass size={14} />
                  {t('management.blueprintList.exploreBlueprints')}
                </Link>
                <button
                  type="button"
                  onClick={() => uploadInputRef.current?.click()}
                ...
                >
                  <UploadSimple size={14} weight="bold" />
                  {uploading ? t('management.blueprintList.uploading') : t('management.blueprintList.uploadGadget')}
                </button>
              </div>
            </div>
          )
