  ...
  return (
    ...
    >
      <button
        ...
        title={expanded ? t('management.workpiece.collapseOutputs') : t('management.workpiece.expandOutputs')}
        aria-label={expanded ? t('management.workpiece.collapseOutputs') : t('management.workpiece.expandOutputs')}
      ...
      >
        {expanded && (
          <span className="text-[11px] font-medium uppercase tracking-[0.06em]">{t('management.workpiece.outputs')}</span>
        )}
      ...
      <div className="flex flex-1 flex-col gap-0.5 overflow-y-auto overflow-x-hidden px-1.5 pb-2">
        {gadgets.map(gadget => {
            ...
            return (
              <div key={gadget.id} className="flex items-center gap-1 py-0.5">
                ...
                <WorkshopIconButton
                  ...
                  className="!h-6 !w-6"
                  aria-label={t('management.workpiece.saveName')}
                >
                  <Check size={13} />
                </WorkshopIconButton>
                <WorkshopIconButton
                  ...
                  className="!h-6 !w-6"
                  aria-label={t('management.workpiece.cancelRename')}
                >
                  <X size={13} />
                </WorkshopIconButton>
              </div>
          ...
          return (
            ...
            >
              <Tooltip content={`${gadget.title}${!expanded && isPending ? ` (${t('management.workpiece.draft')})` : ''}${hasHook ? ` · ${t('management.workpiece.hooksEnabled')}` : ''}`} asChild>
                <button
                  type="button"
                ...
                >
                  ...
                  {hasHook && (
                    <span
                      role="img"
                      aria-label={t('management.workpiece.hooksEnabled')}
                      className={expanded
              ...
              {expanded && (
                <WorkshopIconButton
                  ...
                  title={t('management.workpiece.renameGadget')}
                  aria-label={t('management.workpiece.rename', { title: gadget.title })}
                >
        ...
        <Tooltip content={t('management.workpiece.viewActivity')} asChild>
          <button
            type="button"
          ...
          >
            ...
            {expanded && <span className="min-w-0 flex-1 truncate">{t('management.workpiece.viewActivity')}</span>}
            {expanded ? (
