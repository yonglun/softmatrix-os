...
function GadgetUISession({ gadget, height, reloadTrigger, isVisible = true, chatId, onConsoleLog, onIframeEscape }: GadgetUIProps) {
  ...
  useEffect(() => {
    ...
    const giveUp = setTimeout(() => {
      ...
      setLoading(false)
      setError(t('management.gadgetUi.loadFailed'))
    ...
    const handleMessage = async (event: MessageEvent) => {
      ...
      if (event.data === 'handshake' && event.ports && event.ports[0]) {
        ...
        try {
          ...
          setError(t('management.gadgetUi.connectFailed'))
        } finally {
    ...
    return (
      ...
      >
        <Text variant="secondary">
          {t('management.gadgetUi.switchTab')}
        </Text>
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div style={{
        ...
        <Banner
          variant="error"
          title={t('management.gadgetUi.error')}
          description={error}
          action={
            <Banner.Action
              onClick={() => {
            ...
            >
              {t('management.gadgetUi.tryAgain')}
            </Banner.Action>
          }
        />
      </div>
  ...
  return (
    <div style={{ height, width: '100%' }}>
      <iframe
        ...
        title={t('management.gadgetUi.title')}
      />
    </div>
  )
}
